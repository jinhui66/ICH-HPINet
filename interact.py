...
parser.add_argument('--int_weight', type=str, default="./checkpoints/int_model.pth")
...
interaction_model = deeplabv3_resnet50(num_classes=1, output_stride=16, pretrained_backbone=False)
...
processor = iMIS(seg3d_model.cuda(), interaction_model.cuda(), propagation_model.cuda(), f3d_converter=convert_model.cuda())

save_final_mask = False
save_round_mask = False
data_dir="../DATA/private_ICH"
ori_data_dir="../DATA/private_ICH"
val_set='./eval.txt'
shuffle=False
max_time=None
max_nb_interactions=20
...
with torch.no_grad():
    ...
    with KiTSInteractiveSession(data_dir=data_dir,val_set=val_set,shuffle=shuffle,max_time=max_time,max_nb_interactions=max_nb_interactions,
        ...
        while sess.next():
            ...
            # np.save(os.path.join(debug_dir,"{}_scribble.npy".format(r)), scribble[150])
            # np.save(os.path.join(debug_dir,"{}_pred.npy".format(r)), pred_masks[150])
