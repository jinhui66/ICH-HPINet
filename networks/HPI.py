...
class HPI:
    def __init__(self, seg3d_model, seg2d_model, prop_model, volumn=None, num_objects=1, gpu=True, f3d_converter=Converter()):
        '''
    ...
    def interact(self, scribble_mask, only_int=False, final_pred="prop", only_3d=False):
        ...
        self.pred_masks[idx] = self.run_2d_model(int_slice, scribble_mask, prev_r_mask).cpu()
    ...
    def run_2d_model(self, image, scribble_mask, prev_round_mask, case_ID = None, slice_idx=None, first_ite = None):
        '''
        input size: (b, 1, H, W)
        ...
        mask = torch.sigmoid(self.seg2d_model(inputs))
