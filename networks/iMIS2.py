...
class iMIS:
    ...
    def run_3d_model(self, scribble_3d, crop_first=True, only_3d=False):
        ...
        # if only_3d:
        out, self.features_3d = self.seg3d_model(torch.cat([self.volume_patch, self.pred_patch_mask],dim=1))
        # else:
        #     if self.current_round<=1:
        #         # print(self.volume_patch, self.pred_patch_mask, scribble_3d)
        ...
        #         patch_mask = torch.zeros(size).to(self.device)
        #         patch_mask[0,0,:corners[3]-corners[0]] = self.pred_masks[corners[0]:corners[3], corners[1]:corners[4], corners[2]:corners[5]]
        #         patch_mask = F.interpolate(patch_mask, size=[self.crop_size, self.crop_size, self.crop_size], mode="trilinear", align_corners=False)
    ...
    def decode_3d_feature(self, slice_idx, pred=False): # convert 3d feature to 2d slice feature
        ...
        f_3d, preds = self.f3d_converter.module.convert_3d_feature(f_3d, pos_vectors, pred_mask=pred)
    ...
    def run_int_model(self, image, scribble_mask, prev_round_mask = None, case_ID = None, slice_idx=None, first_ite = None):
        '''
        input size: (b, 1, H, W)
        ...
        inputs = torch.cat([image, scribble_mask], 1) # (b, 3, H, W)
        if inputs.shape[1] != 3:
            print("Unexpected input channel number, expect 3, got ", inputs.shape[1])
    ...
    def run_prop_model(self, target_image, f_3d, segmented_images, segmented_masks, clear=True, additional_k_v = None):
        ...
        q_k, q_v, q_f16, q_f8, q_f4 = self.prop_model.module.encode_key(target_image)
        ...
        if segmented_images is not None:
            ...
            for i in range(k):
                ...
                m_k, _, m_f16, _, _ = self.prop_model.module.encode_key(memory_frame) # m_k: b*64*h/16*w/16
                m_v = self.prop_model.module.encode_value(memory_frame, m_f16, memory_mask) # m_v: b*512*1*h/16*w/16
        ...
        pred_mask, affinity = self.prop_model.module.segment_with_query(memory_keys, memory_values, q_f8, q_f4, q_k, q_v, f_3d)
    ...
    #         img_list[i] = torch.from_numpy(img_list[i].astype(float))
    #         if self.gpu:
    ...
    def crop_3d_patch(self, scribble_3d):
        ...
        region = torch.where(self.pred_masks>0.5)
        ...
        crop_length_hw, crop_length_z = [self.crop_size, self.crop_size]

        crop_length_hw = max(max(lh,lw)+10, self.crop_size)
        crop_length_z = lz + 20
        c1 = corners.clone()
        ...
        volume_patch = torch.zeros(size).to(self.device)
        volume_patch[:corners[3]-corners[0]] = self.volumn[corners[0]:corners[3], corners[1]:corners[4], corners[2]:corners[5]]
        scribble = torch.zeros([1,2]+size).to(self.device)
        scribble[:,:,:corners[3]-corners[0]] = scribble_3d[:,:,corners[0]:corners[3], corners[1]:corners[4], corners[2]:corners[5]]
        scribble_3d = scribble

        if self.pred_patch_mask is None:
            self.pred_patch_mask = torch.zeros(1,1,self.crop_size,self.crop_size,self.crop_size).to(self.device)
        else:
            pred_patch_mask = torch.zeros(size)
            pred_patch_mask[:corners[3]-corners[0]] = self.pred_mask_seg3d[corners[0]:corners[3], corners[1]:corners[4], corners[2]:corners[5]]
            pred_patch_mask = pred_patch_mask.unsqueeze(0).unsqueeze(0)
            self.pred_patch_mask = F.interpolate(pred_patch_mask, size=[self.crop_size, self.crop_size, self.crop_size], mode="trilinear", align_corners=False).to(self.device)
        
        self.crop_info = {"ori_crop_size":volume_patch.shape, "volume_size":self.volumn.shape, "crop_range":corners, "z_padding":z_padding}

        volume_patch = volume_patch.unsqueeze(0).unsqueeze(0)
        self.volume_patch = F.interpolate(volume_patch, size=[self.crop_size, self.crop_size, self.crop_size], mode="trilinear", align_corners=False).to(self.device)
        scribble_3d = F.interpolate(scribble_3d, size=[self.crop_size, self.crop_size, self.crop_size], mode="trilinear", align_corners=False)
