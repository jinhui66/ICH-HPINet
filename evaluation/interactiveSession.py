...
class KiTSInteractiveSession:
    ...
    def next(self):
        ...
        if sample_change:
            ...
            if self.sample_idx < len(self.val_data_index_list):
                ...
                self.volume = self.volume[len(self.volume) // 2 - 8 : len(self.volume) // 2 + 8]
                self.gt_mask = self.gt_mask[len(self.gt_mask) // 2 - 8 : len(self.gt_mask) // 2 + 8]
        ...
        #     # np.save("./pred_mask.npy", self.pred_mask)
    ...
    def get_interaction_report(self):
        ...
        # print(interaction, self.eval_score_slice, self.interaction_slice)
    ...
    def evaluate_mask(self, pred_mask):
        ...
        if self.metric_to_optimize == "Dice":
            ...
            gt_mask = torch.from_numpy(self.gt_mask).float()
            pred_mask = torch.from_numpy(pred_mask).float()
            sample_dice = dice(gt_mask, pred_mask, thresh=self.thresh)

            for i in range(pred_mask.shape[0]):
                slice_dice = dice(gt_mask[i], pred_mask[i], thresh=self.thresh)
                self.eval_score_slice.append(slice_dice)
                    
                slice_jaccard = jaccard(gt_mask[i], pred_mask[i])
                slice_hd = HD(gt_mask[i], pred_mask[i])
                slice_mae = MAE(gt_mask[i], pred_mask[i])

                
                dice_list.append(slice_dice)
                jaccard_list.append(slice_jaccard)
                hd_list.append(slice_hd)
        ...
        # print(self.interaction_slice[self.interaction_nb-1])
        # print("Dice:", dice_list, sample_dice)
        # print("Jaccard:", jaccard_list, sample_jaccard)
        # print("HD:",hd_list, sample_hd)
        # print("MAE:",mae_list, sample_mae)
        # print("Dice:", sample_dice)
        # print("Jaccard:", sample_jaccard)
        # print("HD:", sample_hd)
