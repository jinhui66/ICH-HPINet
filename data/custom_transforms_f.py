    ...
    def __call__(self, sample):
        for elem in sample.keys():
            if 'image' in elem:
                ...
                sample[elem] = np.subtract(sample[elem], np.array(self.mean, dtype=float))
        return sample
    ...
    def __call__(self, sample):
        ...
        for scribble in self.scribbles[frame_num]:
            points_scribble = np.round(
                np.array(scribble['path']) * np.array((im_size[1], im_size[0]))).astype(int)
            ...
            if not self.use_previous_mask:
                ...
                scr_nocare_extra = \
                    (cv2.imread(os.path.join(self.previous_mask_path, meta['seq_name'], str(meta['obj_id']),
                                             meta['frame_id'] + '.png'), 0) > 0.8 * 255).astype(float)
